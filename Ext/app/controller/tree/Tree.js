Ext.define("Demo.controller.tree.Tree", {
    ...
    onCheckChange: function (node, ischecked) {
        ...
        checkedChildNode(node, ischecked);
        checkedParentNode(node, ischecked);
    }
});

// 选中所有的子节点
var checkedChildNode = function (node, ischecked) {
    node.set("checked", ischecked);
    node.set("cls", "");
    if (node.hasChildNodes()) {     // 是否包含子节点
        node.eachChild(function (nodeChild) {
            checkedChildNode(nodeChild, ischecked)
        });
    }
}
...
var checkedParentNode = function (node, ischecked) {
    ...
    if (parent != null) {
        ...
        checkedParentNode(parent, ischecked);
    }
}
